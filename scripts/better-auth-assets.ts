...
import { betterAuthAdapter } from '../src/integrations/auth/better-auth/index.js';
...
for (const example of ['messages', 'none'] as const) {
  const root = await mkdtemp(join(tmpdir(), `ccm-better-auth-${example}-`));
  ...
  await betterAuthAdapter.apply(context);
  const assets = fileURLToPath(
    new URL(
      `../assets/backend${example === 'none' ? '-blank' : ''}-better-auth/`,
      import.meta.url,
    ),
  );
  const backend = join(root, 'packages/backend');
  await mkdir(join(assets, 'convex'), { recursive: true });
  for (const module of [
    'schema.ts',
    ...(example === 'messages' ? ['messages.ts'] : []),
  ]) {
    await cp(join(backend, 'convex', module), join(assets, 'convex', module));
  }
  ...
  if (!api.includes('betterAuth'))
    throw new Error(`Codegen omitted the Better Auth component in ${root}.`);
  ...
  await writeFile(
    ...
    `Generated files under convex/\\_generated were produced unchanged by convex@${versions.convex} using CONVEX_AGENT_MODE=anonymous pnpm exec convex dev --once on ${new Date().toISOString().slice(0, 10)}, after pnpm install in a temporary generated workspace. Dynamic JS/declaration output. Auth dependencies: @convex-dev/better-auth@${versions.convexBetterAuth}, better-auth@${versions.betterAuth} and @better-auth/expo@${versions.betterAuthExpo}. Input modules: convex.config.ts, schema.ts, auth.ts, http.ts${example === 'messages' ? ', messages.ts, access.ts' : ''}. auth.config.ts configures the issuer and is excluded from the generated function API. No internal codegen entry point was called. Run normal convex dev after changing backend modules.\n\nConvex SDK source is Apache-2.0 licensed: https://github.com/get-convex/convex-js/blob/main/LICENSE.\n`,
  );
}
