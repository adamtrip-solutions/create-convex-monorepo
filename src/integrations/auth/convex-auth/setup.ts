...
function oauthSetup(scope: string, oauth: readonly OAuthProvider[]): string {
  return `### OAuth setup
...
\`\`\`sh
${oauth.map((provider) => `pnpm --filter @${scope}/backend exec convex env set AUTH_${provider.toUpperCase()}_ID <client-id>\npnpm --filter @${scope}/backend exec convex env set AUTH_${provider.toUpperCase()}_SECRET <client-secret>`).join('\n')}
\`\`\`
