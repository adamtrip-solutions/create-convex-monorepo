...
This starter uses the official Convex Better Auth component with email and password. Sign-up uses the email address as the account name. Email verification and social providers are disabled. Next.js, TanStack Start, and React Router use client authentication only. Astro uses the same browser client inside its React island.
...
Convex supplies the server-side CONVEX_SITE_URL. BETTER_AUTH_SECRET stays on the deployment and must never have a NEXT_PUBLIC_, VITE_, PUBLIC_, or EXPO_PUBLIC_ prefix. The component creates its own signing keys; no manual JWKS setting is required.

Expo uses expo-secure-store to persist sessions on native devices. Build a development client from the Expo app directory with ${run('ios')} or ${run('android')} so the configured ccm-${options.name}-<app> scheme is available. Add each scheme to the deployment's trusted origins. A phone needs reachable deployment URLs. Its localhost refers to the phone itself. Web clients use the cross-domain plugin; native clients use the Expo plugin.
