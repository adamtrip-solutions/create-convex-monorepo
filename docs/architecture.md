...
An auth adapter implements `apply(context)`. It declares `supportedRuntimes` and owns backend `access.ts`, optional auth configuration, and the frontend provider and auth controls. React apps use `providers.tsx` and `auth-controls.tsx`; Svelte apps use `Providers.svelte` and `AuthControls.svelte`. The framework-to-runtime mapping lives in `src/integrations/auth/shared.ts`. Normalization and auth installation validate these capabilities before writing files. Framework templates import those stable module names without importing Clerk. React Router keeps its upstream `app/` routes and imports the shared modules from `src/`. Its root route re-exports auth-adapter-owned `src/auth.server.ts` hooks for Clerk SSR session loading, or null data and empty middleware for client-only auth. Clerk's binding map selects the framework SDK, middleware and native dependencies in `src/integrations/auth/clerk/index.ts`. Adding a provider requires platform-specific integration, but does not require embedding provider branches throughout framework templates.
...
The package-manager contract lives in `src/package-manager/index.ts`. The selected pnpm or bun adapter owns installation; subprocesses receive argument arrays and inherited terminal output. The root template owns Turborepo and workspace configuration. There is no Nx adapter or speculative migration engine.
...
Better Auth registers the official component through `convex/convex.config.ts`. Its adapter owns `auth.ts`, `auth.config.ts`, `http.ts`, the environment setup helper, and frontend auth modules. Both starters ship official generated component references. The component keeps auth tables separate from the application schema; messages use the Better Auth user ID as their owner. Browser clients use the cross-domain plugin, while Expo selects SecureStore on native platforms.

Blank apps do not include tests that assume an empty API forever. The generated-project test runner injects temporary compile-time assertions for the selected provider's API and table list, rejects unknown modules, and checks for widened types. Those files belong only to the disposable test fixture.
...
`project.ts` walks upward to metadata, validates version 1, retains extension fields, and rejects unsafe file paths. `add.ts` renders temporary scaffolds with the existing app/auth adapters, then selects only the new app or authentication changes. Adding Astro also upgrades a recognized older URL-link helper and adds public environment settings and output ignores to existing root configuration. These changes use the same guarded plan; a conflicting setup helper stops the addition before writes. Auth package updates merge only the fields changed by the adapter; source files must match the expected baseline before replacement. Comparison normalizes formatting with the same formatter to support older unformatted templates, preserving meaningful code and comment differences as conflicts. Apply-time snapshots still compare exact contents. Per-app example overrides survive later auth additions.

`add package <name>` uses the same plan, guards, and apply flow to create a blank shared TypeScript package. It formats new files with the generator formatter and appends a `{ name }` entry to the optional version 1 metadata `packages` list, preserving unknown fields. Next configs are updated when they match the generated config apart from the `transpilePackages` string list, so repeated `add package` calls keep appending. A missing config or any other customization produces a note with the required manual edit. Other frameworks need no config change.
