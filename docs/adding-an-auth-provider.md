...
4. For each app, merge SDK dependencies through `context.mergePackage` and write the components for its supported UI runtime. React uses `src/providers.tsx` and `src/auth-controls.tsx`; Svelte uses `src/Providers.svelte` and `src/AuthControls.svelte`. Nuxt uses `src/components/Providers.vue` and `src/components/AuthControls.vue`, with client setup in `src/plugins/convex.client.ts`. SvelteKit and Nuxt currently support only the none adapter. Add middleware or native configuration where the SDK requires it.
...
WorkOS AuthKit is the third adapter and supports Next.js, Vite, TanStack Start, and Expo. It owns a ConvexProviderWithAuth token bridge, server callback routes where required, and `.env.workos.example`. Expo has no official SDK; `src/integrations/auth/workos/native.ts` writes a PKCE public client built on `expo-auth-session` with SecureStore token storage and a single-flight refresh. SvelteKit is rejected by the runtime compatibility check. Astro and React Router are rejected during normalization through the WorkOS bindings map. Astro has an [official SDK](https://github.com/workos/authkit-astro), but it requires a server adapter and on-demand rendering, which the static Astro template does not configure. The official React Router SDK exists, but this generator has no binding for it yet. Ownership uses `identity.subject`, unlike Clerk and Convex Auth.
